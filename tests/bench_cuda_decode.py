...
sp_cu = NDArray(np.abs(np.random.randn(1, HIDDEN).astype(np.float32)), device=dev_cu)
sp_np = NDArray(np.abs(np.random.randn(1, HIDDEN).astype(np.float32)), device=dev_np)
t_pow_cu = time_op(lambda: sp_cu ** 0.5, 500)
t_pow_np = time_op(lambda: sp_np ** 0.5, 500)
