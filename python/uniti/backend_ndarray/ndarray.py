...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        return NDArray.make(new_shape,NDArray.compact_strides(new_shape),self.device,self._handle)
    ...
    def broadcast_to(self, new_shape):
        ...
        for i, (a, b) in enumerate(zip(self.shape, new_shape)):
          if (a!=1):
            assert(a == b)
        l = list(self.strides)
        for i, (a, b) in enumerate(zip(self.shape, new_shape)):
          if a != b:
            l[i] = 0
        return NDArray.make(new_shape, tuple(l), self.device, self._handle, self._offset)
    ...
    def __getitem__(self, idxs):
        ...
        new_shape = tuple([(sl.stop -sl.start + sl.step-1)//sl.step for sl in slices])
        new_stride = tuple([self.strides[i] * slices[i].step  for i in range(len(self.strides))])
    ...
    def flip(self, axes):
        ...
        for axis in axes:
          new_off += self.strides[axis] * (self.shape[axis] - 1)
          new_strides[axis] = -new_strides[axis]
        out = NDArray.make(self.shape, new_strides, self.device, self._handle, new_off).compact()
        return out
    ...
    def pad(self, axes):
        ...
        assert len(axes) == len(self.shape), "axes's length should be the same as ndarray's shape"
        new_shape = tuple([l + r + n for (l, r), n in zip(axes, self.shape)])
        new_array = full(new_shape, 0, device=self.device)
        idxs = tuple([slice(l, l+n) for (l,r),n in zip(axes, self.shape)])
        new_array[idxs] = self
        return new_array
