...
class TensorTuple(Value):
    ...
    def __getitem__(self, index: int):
        return uniti.ops.tuple_get_item(self, index)

    def tuple(self):
        return tuple([x for x in self])

    def __repr__(self):
        return "uniti.TensorTuple" + str(self.tuple())
    ...
    def __add__(self, other):
        ...
        return uniti.ops.make_tuple(*[self[i] + other[i] for i in range(len(self))])
...
class Tensor(Value):
    ...
    def __repr__(self):
        return "uniti.Tensor(" + str(self.realize_cached_data()) + ")"
    ...
    def __add__(self, other):
        if isinstance(other, Tensor):
            return uniti.ops.EWiseAdd()(self, other)
        else:
            return uniti.ops.AddScalar(other)(self)

    def __mul__(self, other):
        if isinstance(other, Tensor):
            return uniti.ops.EWiseMul()(self, other)
        else:
            return uniti.ops.MulScalar(other)(self)

    def __pow__(self, other):
        if isinstance(other, Tensor):
            return uniti.ops.EWisePow()(self, other)
        else:
            return uniti.ops.PowerScalar(other)(self)

    def __sub__(self, other):
        if isinstance(other, Tensor):
            return uniti.ops.EWiseAdd()(self, uniti.ops.Negate()(other))
        else:
            return uniti.ops.AddScalar(-other)(self)

    def __truediv__(self, other):
        if isinstance(other, Tensor):
            return uniti.ops.EWiseDiv()(self, other)
        else:
            return uniti.ops.DivScalar(other)(self)

    def __matmul__(self, other):
        return uniti.ops.MatMul()(self, other)

    def matmul(self, other):
        return uniti.ops.MatMul()(self, other)

    def sum(self, axes=None):
        return uniti.ops.Summation(axes)(self)

    def broadcast_to(self, shape):
        return uniti.ops.BroadcastTo(shape)(self)

    def reshape(self, shape):
        return uniti.ops.Reshape(shape)(self)

    def __neg__(self):
        return uniti.ops.Negate()(self)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
      ajoint = sum_node_list(node_to_output_grads_list[i])
      i.grad = ajoint
      if i.op is None:
        continue
      input_ajoints = i.op.gradient_as_tuple(ajoint,i)
      for k, input_ajoint in zip(i.inputs, input_ajoints):
        if k not in node_to_output_grads_list:
          node_to_output_grads_list[k] = []
        node_to_output_grads_list[k].append(input_ajoint)
